...
from control.lqr import VeloLQR, HeadingLQR
...
if __name__ == "__main__":
    ...
    # set this value to 0 to disable hydro param deviation
    ...
    simulator = NoisyVesselSimulator(
        ...
        # #the line below to disable tau noise
        tau_noise_gen=tau_noise_gen,
    )
    thruster = NaiveDoubleThruster(b=sample_b_2)

    diff_controller = HeadingLQR(
        m33=noised_hydro_params["m33"],
        d33=noised_hydro_params["N_r"],
        b=sample_b_2,
        Q=Q,
        R=R,
        control_lim=max_diff_N/2,
    )
    u_controller = VeloLQR(
        m11=noised_hydro_params["m11"],
        d11=noised_hydro_params["X_u"],
        Q_err=Q_uerr,
        R_F=R_uF,
        control_lim=max_base_N,
    )
    guider = LOSGuider(waypoints=waypoints, reached_threshold=2, output_err_flag=False)
    ...
    for t in range(total_exp_steps):
        ...
        if t % control_every == 0:
            ...
            diff_control_signal = diff_controller.step(error_psi=psi_err, r=current_r)
            velo_control_signal = u_controller.step(u=current_u, tgt_u=desire_u)
            # truncate
    ...
    psis=np.array(psis)
    psis = (psis + 180) % 360 - 180
    psi_errs=np.array(psi_errs)
    psi_errs = (psi_errs + 180) % 360 - 180
