...
if __name__ == "__main__":
    ...
    guider = LOSGuider(waypoints=waypoints, reached_threshold=reached_threshold, output_err_flag=False)
    ...
    ax.plot(way_ys, way_xs, "--", color="red")
    ax.plot(ys, xs, label="Naive LOS", color="#9EC9E6", alpha=0.9)
    ...
    guider = FixedDistLOSGuider(waypoints=waypoints, reached_threshold=reached_threshold, output_err_flag=False, los_dist=2.5)
    xs, ys = test_guider(simulator, thruster, diff_controller, u_controller, guider,
                current_state, total_exp_steps, control_every)

    ax.plot(ys, xs, label="Fixed LOS", color="#3E92CC", alpha=0.9)
    ...
    guider = DynamicDistLOSGuider(waypoints=waypoints, reached_threshold=reached_threshold, output_err_flag=False, forward_dist=forward_dist)
    xs, ys = test_guider(simulator, thruster, diff_controller, u_controller, guider,
                current_state, total_exp_steps, control_every)

    ax.plot(ys, xs, label="Dynamic LOS", color="#1C4A69", alpha=0.9)
