...
class RudderLaser(QGraphicsView):
    def __init__(self, parent=None):
        ...
        self.m_originalPropCtr = QPointF(120.0, 55)

        self.m_rudder = 0
        self.m_itemRudder = None
        self.m_scaleX = None
        self.m_scaleY = None
    ...
    def getRudder(self) -> float:
        return self.m_rudder

    def setRudder(
        self,
        rudder: float,
    ):
        self.m_rudder = rudder
        self.update()
    ...
    def init(self):
        ...
        self.m_itemRudder = QGraphicsSvgItem("./USVWidgets/svg/rudder_laser.svg")
        self.m_itemRudder.setCacheMode(QGraphicsItem.CacheMode.NoCache)
        #self.m_itemRudder.setZValue(self.m_faceZ)
        self.m_itemRudder.setTransform(
            QTransform.fromScale(self.m_scaleX, self.m_scaleY),
            True,
        )
        self.m_itemRudder.setTransformOriginPoint(self.m_originalPropCtr)
        self.m_scene.addItem(self.m_itemRudder)

        super().centerOn(self.width()/2, self.height()/2)
        self.updateView()

    def updateView(self):
        self.m_itemRudder.setRotation(-self.m_rudder)
        self.m_scene.update()